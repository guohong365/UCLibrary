...
double MathUtility::CalculateLineAngle(double x1, double y1, double x2, double y2)
{
    if (y1 == y2)
    {
        if (x1 <= x2)
...
bool MathUtility::CalculateCorrectParameter(const double *x0, const double *y0, const double *x1, const double *y1, double *k)
{
    bool quit = false;
...
void MathUtility::CalculatePrespectiveCoordinate(const double * k, double x0, double y0, double &x, double &y)
{
    x = (x0 * k[0] + y0 * k[1] + k[2])/(k[6] * x0 + k[7] * y0 + 1);
    y =	(x0 * k[3] + y0 * k[4] + k[5])/(k[6] * x0 + k[7] * y0 + 1);
}
void MathUtility::CalculatePrespectiveCoordinate(const double * k, const double *x0, const double *y0, double * x, double *y, long count)
{
